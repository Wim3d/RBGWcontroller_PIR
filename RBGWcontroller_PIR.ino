/*
   ...
   May 2020
   RGBW LED strip controller Magic home ESP8285, based on WiFi LED bulb software
*/
...
const char* devicename = "RGBWLEDstrip1";
...
int WWLedValint = 100;  // start with WW light when powered on

int prev_redLedValint, prev_greenLedValint, prev_blueLedValint, prev_WWLedValint;
...
void loop() {
  ...
  {
    //client.publish(status_topic, "GPIO4 = HIGH");
    timer = millis();
    if (!stripon)
      restorestrip();
    if (!published)
    {
      client.publish(status_topic, "motion detected");
      published = true;
    }
  }
  else if (published)   // PIRPIN = LOW, reset published
    published = false;

  if (stripon && millis() > timer + LED_DELAY_S * 1000)
    stripoff();
...
void callback(char* topic, byte * payload, unsigned int length)
{
  ...
  if ((char)topic[14] == 'D') // search for   RGBWLEDstrip1/DIM
  {
  ...
  else
  {
    ...
    if ((char)topic[14] == 'R') // search for   RGBWLEDstrip1/RGB
    {
      scene = 0;
    ...
    else if ((char)topic[14] == 'W') // search for   RGBWLEDstrip1/WW
    {
      scene = 0;
    ...
    else if ((char)topic[15] == 'c') // search for   RGBWLEDstrip1/scene#
    {
      state = 0;
      scene = (char)topic[19] - '0';
    }
  }
...
boolean reconnect()
{
  ...
  if (client.connect(host)) {
    ...
    client.subscribe("RGBWLEDstrip1/scene1");
    client.subscribe("RGBWLEDstrip1/scene2");
    client.subscribe("RGBWLEDstrip1/scene3");
    client.subscribe("RGBWLEDstrip1/scene4");
...
void handle_scene1() {
  client.publish("RGBWLEDstrip1/scene1", "ON");
  httpServer.send(200, "text/html", SendHTML());
}

void handle_scene2() {
  client.publish("RGBWLEDstrip1/scene2", "ON");
  httpServer.send(200, "text/html", SendHTML());
}

void handle_scene3() {
  client.publish("RGBWLEDstrip1/scene3", "ON");
  httpServer.send(200, "text/html", SendHTML());
}
...
void stripoff(void)
{
  client.publish(status_topic, "stripoff");
  stripon = false;
  prev_blueLedValint = blueLedValint;
  prev_redLedValint = redLedValint;
  prev_greenLedValint = greenLedValint;
  prev_WWLedValint = WWLedValint;
  blueLedValint = redLedValint = greenLedValint = WWLedValint = 0;
  updateLED();
}

void restorestrip(void)
{
  client.publish(status_topic, "restorestrip");
  stripon = true;
  blueLedValint = prev_blueLedValint;
  redLedValint = prev_redLedValint;
  greenLedValint = prev_greenLedValint;
  WWLedValint = prev_WWLedValint;
  updateLED();
}
